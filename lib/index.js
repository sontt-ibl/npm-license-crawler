        ...
        .on("done", function (error) {
            if (! error) {
                async.eachSeries(filePaths, function (filePath, iteratorCallback) {
                    ...
                    checker.init(args, function(error, json) {
                        if(!!error) {
                    ...
                    if (error) {
                        ...
                        if (args.json || args.csv) {
                            ...
                            if (args.csv) {
                                dir = path.dirname(args.out);
                                mkdirp.sync(dir);
                                fs.writeFileSync(args.csv, (checker.asCSV(result, args.customFormat, args.csvComponentPrefix) + '\n'), 'utf8');
